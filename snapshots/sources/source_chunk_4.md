...
# FILE: tests/scripts/test_generate_cli_guide.py
```python
...
def test_cli_help_written(tmp_path, monkeypatch):
    ...
    assert content.startswith("# DMForge CLI Usage Guide")
```
...
# FILE: tests/scripts/test_update_docs.py
```python
import importlib.util
from pathlib import Path
...
def test_skips_already_versioned_file(tmp_path):
    ...
    try:
        ...
        assert False, "Expected ValueError on already versioned file"
...
# FILE: tests/scripts/test_validate_docs.py
```python
import subprocess
...
# FILE: tests/scripts/test_version_docs.py
```python
